...
class KelompokApi {
  ...
  Future insertKelompok(KelompokModel kelompok, File fotoProfilImage) async {
    try {
      if (_serverAddress == null) {
        await getPrefs();
      }

      Map<String, dynamic> bodyjson = kelompok.toJson();
      // Map<dynamic, dynamic> formFields = {};

      var url = Uri.parse("$_serverAddress/kelompok");
  ...
  Future updateKelompok(KelompokModel kelompok, File fotoProfilImage) async {
    try {
      if (_serverAddress == null) {
        await getPrefs();
      }
      ...
      var url = Uri.parse("$_serverAddress/kelompok/update");
      ...
      bodyjson.forEach((k, v) {
        switch (v.runtimeType.toString()) {
          case "List<dynamic>": 
            ...
            request.fields[k] =json.encode( v);
            break;
          case "List<KelompokAnggotaModel>": 
            print("List<KelompokAnggotaModel>")
  ...
  Future<List<KelompokModel>> selectListByNama(String namaKelompok) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    ...
    try {
      ...
      final url =
          '$_serverAddress/kelompok/select_by_name_with_anggota/$namaKelompok';

      final response = await client
          .get(Uri.encodeFull(url), headers: {'Accept': "application/json"});
      if (response.statusCode == 200) {
        return parseKelompokList(response.body);
  ...
  Future<List<KelompokModel>> fetchBirthdays(String bulan) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    if (bulan.trim() == '') {
    ...
    try {
      final client = new http.Client();
      final url = '$_serverAddress/kelompok/select_hut_by_bulan/$bulan';

      final response = await client
          .get(Uri.encodeFull(url), headers: {'Accept': "application/json"});
      if (response.statusCode == 200) {
        return parseKelompokList(response.body);
        // return compute(parseKelompokList, response.body);
      }
  ...
  List<KelompokModel> parseKelompokList(String responseBody) {
    ...
    /// example call : return compute(parseKelompokList, response.body);
    final dataMap =
    ...
    return dataMap
        .map<KelompokModel>((json) => KelompokModel.fromJson(json))
        .toList();
  }

  Future insertKelompokHttpPost(KelompokModel kelompok) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
...
// EROR: Bad state: Cannot set the body fields of a Request with content-type "multipart/form-data"
  ...
  // Base64Codec _toJson(KelompokModel kelompok) {
  //   var mapData = new Map();
  //   mapData["name"] = kelompok.name;
  //   mapData["dob"] = new DateFormat.yMd().format(kelompok.dob);
  //   mapData["phone"] = kelompok.phone;
  //   mapData["email"] = kelompok.email;
  //   mapData["favoriteColor"] = kelompok.favoriteColor;
  //   String jsonstr = json.encode(mapData);
  //   return jsonstr;
  // }
  Future<KelompokModel> loginKelompok(String email, String password) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    final response =
        await _client.get('$_serverAddress/kelompok/$email&$password');
    if (response.statusCode == 200) {
      Map<String, dynamic> mapResponse = json.decode(response.body);
      if (mapResponse["status"] == "ok") {
        final userData =
            mapResponse["data"]["document"].cast<Map<String, dynamic>>();
        return userData.map<KelompokModel>((json) {
          return KelompokModel.fromJson(json);
        });
      } else {
        return null;
      }
    ...
    //   .then((userDataMap) => Kelompok.fromJson(userDataMap))
    //   .catchError(onError);
  }

  Future<List<KelompokModel>> fetchKelompokList(String userGroup) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    final response = await _client.get("$_serverAddress/kelompok");
    ...
    if (response.statusCode == 200) {
      ...
      if (mapResponse["status"] == "ok") {
        ...
        //final listOfKelompok = await Future.wait(futures)

        final listOfKelompok = await rincianKegiatan.map<KelompokModel>((json) {
          return KelompokModel.fromJson(json);
        }).toList();

        // print("AFTER ===");
        return listOfKelompok;
      } else {
  ...
  Future<KelompokModel> deleteKelompok(
      http.Client client, String idKegiatan, String email) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    final String url = '$_serverAddress/kelompok/$idKegiatan&$email';

    final response = await client.delete(url);

    if (response.statusCode == 200) {
      final responseBody = await json.decode(response.body);
      return KelompokModel.fromJson(responseBody);
    } else {
      throw Exception(
          "Failed t0 delete a Kelompok. Error : ${response.toString()}");
    }
  }

  Future<KelompokModel> signKelompokUp(
      http.Client client, String email, String password) async {
    var body = {"email": email, "password": password};
    final response =
        await client.post('$_serverAddress/kelompok/login', body: body);

    if (response.statusCode == 200) {
      Map<String, dynamic> mapResponse = await json.decode(response.body);

      // return rincianKegiatans.map<Kelompok>((json) {
      //     return Kelompok.fromJson(json);
      //   }).toList();

      return KelompokModel.fromJson(mapResponse);
    } else {
      throw Exception("Failed to login. Error : ${response.toString()}");
    }
  }

  Future insertAnggota(KelompokAnggotaModel anggota) async {
    try {
      if (_serverAddress == null) {
        await getPrefs();
      }
      ...
      var url = Uri.parse("$_serverAddress/kelompok/insert_anggota");
      var request = http.MultipartRequest("POST", url);
  ...
  String getNetWorkImageUrlAnggota(String imagePartUrl) {
    // Save the user preference
  ...
  Future updateAnggota(KelompokAnggotaModel kelompok) async {
    try {
      if (_serverAddress == null) {
        await getPrefs();
      }

...
class DomainApi {
  ...
  getPrefs() {
    _serverAddress = SharedPrefsHelperStatic.getString(PREFKEY_DOMAINSERVER );
    if (_serverAddress==null) {
      _serverAddress="mockserver";
    }
  }

  final http.Client _client = http.Client();

  Future insertDomain(DomainModel anggota, File fotoProfilImage) async {
    try {
      if (_serverAddress == null) {
        await getPrefs();
      }
  ...
  Future updateDomain(DomainModel anggota, File fotoProfilImage) async {
    try {
      if (_serverAddress == null) {
        await getPrefs();
      }
  ...
  Future<List<DomainModel>> fetchList() async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    if (_serverAddress==null) {
    ...
    if (_serverAddress == "mockserver") { 
    return parseDomainList(json.encode( domainModel.jsonMock));
    } else {
      try {
        final client = new http.Client();
        final url = '$_serverAddress/domain/list';

        final response = await client
            .get(Uri.encodeFull(url), headers: {'Accept': "application/json"});
        if (response.statusCode == 200) {
          return parseDomainList(response.body);
  ...
  List<DomainModel> parseDomainList(String responseBody) {
    ...
    /// example call : return compute(parseDomainList, response.body);
    final dataMap =
    ...
    return dataMap
        .map<DomainModel>((json) => DomainModel.fromJson(json))
        .toList();
  }

  Future insertDomainHttpPost(DomainModel anggota) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
...
// EROR: Bad state: Cannot set the body fields of a Request with content-type "multipart/form-data"
  ...
  // Base64Codec _toJson(DomainModel anggota) {
  //   var mapData = new Map();
  ...
  //   return jsonstr;
  // }
  Future<DomainModel> loginDomain(String email, String password) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    final response =
    ...
    if (response.statusCode == 200) {
      ...
      if (mapResponse["status"] == "ok") {
        ...
        return userData.map<DomainModel>((json) {
    ...
    //   .then((userDataMap) => Domain.fromJson(userDataMap))
    //   .catchError(onError);
  }

  Future<List<DomainModel>> fetchDomainList(String userGroup) async {
    if (_serverAddress == null) {
      await getPrefs();
    }
    ...
    if (response.statusCode == 200) {
      ...
      if (mapResponse["status"] == "ok") {
        ...
        //final listOfDomain = await Future.wait(futures)

        final listOfDomain = await rincianKegiatan.map<DomainModel>((json) {
          return DomainModel.fromJson(json);
        }).toList();
  ...
  Future<DomainModel> deleteDomain(
    ...
    if (response.statusCode == 200) {
      ...
      return DomainModel.fromJson(responseBody);
    } else {
      throw Exception(
          "Failed t0 delete a Domain. Error : ${response.toString()}");
    }
  }

  Future<DomainModel> signDomainUp(
    ...
    if (response.statusCode == 200) {
      ...
      // return rincianKegiatans.map<Domain>((json) {
      //     return Domain.fromJson(json);
      //   }).toList();

      return DomainModel.fromJson(mapResponse);
    } else {

...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
    ...
    controller.addListener(() {
      setState(() {
        ...
        if (animationFadeIn.status == AnimationStatus.completed) {
          controller2.forward();
          // delay before executing controller3
          new Timer(
    ...
    controller2.addListener(() {
      setState(() {
        ...
        if (animationMoveUp.status == AnimationStatus.forward) {
          logoHeight = animationScaleDown.value;
  ...
  void restartAnim() {
    controller3.reverse().then((T) => controller2
        .reverse()
        .then((T) => controller.reverse().then((T) => forwardController())));
  }
